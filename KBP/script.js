...
// Spotify Token
const spotifyPlaylistId = '6ILMv0OO7PAeCVcObJNnRQ'; // Token Spotify tetap
...
function createSpotifyEmbed(playlistId) {
    ...
    // Directly insert iframe with the updated playlist ID
...
function updateStatistics() {
    const studyPercentage = (studyTime * 60 - timeLeft) / (studyTime * 60) * 100;
    const totalTasks = toDoList.children.length;
    const completedTasks = Array.from(toDoList.children).filter((task) => task.classList.contains('completed')).length;
    const taskCompletionPercentage = totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0;

    document.querySelector('.bar.study-time').style.height = `${studyPercentage}%`;
    document.querySelector('.bar.tasks-completed').style.height = `${taskCompletionPercentage}%`;

    document.querySelector('.study-time .stat-label').textContent = `${Math.round(studyPercentage)}% Study Time`;
    document.querySelector('.tasks-completed .stat-label').textContent = `${Math.round(taskCompletionPercentage)}% Tasks Completed`;
}

setInterval(updateStatistics, 1000); // Update stats every second
